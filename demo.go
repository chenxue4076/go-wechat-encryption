...
import (
	"github.com/chenxue4076/go-wechat-encryption"
	"encoding/xml"
	"fmt"
)
...
func main()  {
	...
	nonce := "xxxxxx"
	wxBizMsgCrypt := wxMsgCrypt.Default("pamtest","abcdefghijklmnopqrstuvwxyz0123456789ABCDEFG","wxb11529c136998cb6")
	encryptMsg, errorCode := wxBizMsgCrypt.EncryptMsg(text, timeStamp, nonce)
	if errorCode != 0 {
		fmt.Println("加密失败："+ wxMsgCrypt.WXBizMsgErrorMsg(errorCode))
	}
	fmt.Println("加密后："+encryptMsg)

	//解析xml
	msgEncryptFormat := wxMsgCrypt.MsgEncryptFormat{}
	...
	if errorCode != 0 {
		fmt.Println("解密失败："+ wxMsgCrypt.WXBizMsgErrorMsg(errorCode))
	}
