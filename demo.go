...
func main()  {
	...
	nonce := "xxxxxx"
	wxBizMsgCrypt := msg_encrypt.Default("pamtest","abcdefghijklmnopqrstuvwxyz0123456789ABCDEFG","wxb11529c136998cb6")
	encryptMsg, errorCode := wxBizMsgCrypt.EncryptMsg(text, timeStamp, nonce)
	if errorCode != 0 {
		fmt.Println("加密失败："+ msg_encrypt.WXBizMsgErrorMsg(errorCode))
	}
	fmt.Println("加密后："+encryptMsg)

	//解析xml
	msgEncryptFormat := msg_encrypt.MsgEncryptFormat{}
	...
	if errorCode != 0 {
		fmt.Println("解密失败："+ msg_encrypt.WXBizMsgErrorMsg(errorCode))
	}
